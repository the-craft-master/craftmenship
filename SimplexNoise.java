...
public class SimplexNoise {
   ...
   private SimplexNoise[] octavesArr;
   private double[] frequenciesArr;
   private double[] amplitudesArr;
   private int largestFeature;
   private double persistence;
   private int seed;
   public static int RANDOMSEED=0;
   public static int NUMSWAPS = 500;
  
 
   private short permutations[] = new short[512];
   private short permutationsMod12[] = new short[512];
  ...
  138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180};
    ...
    public SimplexNoise(int largestFeature,double persistence, int seed){
        ...
        octavesArr = new SimplexNoise[numberOfOctaves];
        frequenciesArr = new double[numberOfOctaves];
        amplitudesArr = new double[numberOfOctaves];
        
        for(int i=0;i<numberOfOctaves;i++){
            octavesArr[i]= new SimplexNoise(random.nextInt());
            frequenciesArr[i] = Math.pow(2,i);
            amplitudesArr[i] = Math.pow(persistence,octavesArr.length-i);
        }
    }
    public double getNoise(int x, int y){
        double result=0;
        for(int i=0;i<octavesArr.length;i++){
          result=result+octavesArr[i].noise(x/frequenciesArr[i], y/frequenciesArr[i])* amplitudesArr[i];
        }
        return result;
    }
  ...
  private static class Grad
  {
    ...
    private static final Logger LOG = Logger.getLogger(SimplexNoise.class.getName());
} 